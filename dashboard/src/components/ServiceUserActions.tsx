...
import { AdvancedUserActionPayload, AdvancedUserActionType } from "types/User";
...
const ServiceUserActions = ({ serviceId }: Props) => {
  ...
  const performServiceUserAction = useServicesStore((state) => state.performServiceUserAction);
  ...
  const runAction = async (action: AdvancedUserActionType, extra?: Partial<AdvancedUserActionPayload>) => {
    const key = action;
  ...
  const handleDays = (action: AdvancedUserActionType) => {
    ...
    if (!Number.isFinite(value) || value <= 0) {
      toast({
        title: t("services.userActions.title", "Service user actions"),
        description: t("filters.advancedActions.error.invalidDays", "Enter a positive number of days"),
        status: "warning",
        isClosable: true,
      });
      return;
  ...
  const handleTraffic = (action: AdvancedUserActionType) => {
    ...
    if (!Number.isFinite(value) || value <= 0) {
      toast({
        title: t("services.userActions.title", "Service user actions"),
        description: t("filters.advancedActions.error.invalidGigabytes", "Enter a positive traffic value"),
        status: "warning",
        isClosable: true,
      });
      return;
    ...
    <Box borderWidth="1px" borderRadius="lg" p={4}>
      <Stack spacing={3}>
        <HStack spacing={2}>
          ...
          <Heading size="sm">{t("services.userActions.title", "Service user actions")}</Heading>
        </HStack>
        ...
        <VStack align="stretch" spacing={4}>
          <Box borderWidth="1px" borderRadius="md" p={3}>
            <Stack spacing={2}>
              ...
              <Text fontSize="sm" color="gray.500">
                {t("services.userActions.expire.helper", "Add or remove days for users in this service.")}
              </Text>
              <NumberInput value={days} onChange={(value) => setDays(value)} min={1} step={1} size="sm">
                <NumberInputField placeholder={t("services.userActions.expire.days", "Days")} />
              </NumberInput>
          ...
          <Box borderWidth="1px" borderRadius="md" p={3}>
            <Stack spacing={2}>
              ...
              <Text fontSize="sm" color="gray.500">
                {t("services.userActions.traffic.helper", "Adjust traffic for all users in this service.")}
              </Text>
              <NumberInput
                value={gigabytes}
                ...
                size="sm"
              >
                <NumberInputField placeholder={t("services.userActions.traffic.gigabytes", "Gigabytes")} />
              </NumberInput>
              <HStack spacing={2} flexWrap="wrap">
                <Button
                  size="sm"
                ...
                >
                  {t("services.userActions.traffic.subtract", "Subtract traffic")}
                </Button>
              </HStack>
            </Stack>
          </Box>
          ...
          <Box borderWidth="1px" borderRadius="md" p={3}>
            <Stack spacing={2}>
              ...
              <Text fontSize="sm" color="gray.500">
                {t("services.userActions.status.helper", "Enable or disable all users in this service.")}
              </Text>
              <HStack spacing={2} flexWrap="wrap">
                <Button
                  size="sm"

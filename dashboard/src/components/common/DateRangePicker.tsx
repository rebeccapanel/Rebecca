...
export type DateRangePickerProps = {
  ...
  presets?: Array<{ key: string; label: string; amount: number; unit: "hour" | "day" | "week" | "month" }>;
...
export const DateRangePicker: FC<DateRangePickerProps> = ({
  ...
  const [selectedPreset, setSelectedPreset] = useState<string>(value.presetKey || value.key || defaultPreset);
  ...
  const handleRelativeChange = (value: string) => {
    ...
    onChange({ start, end, presetKey: "custom", key: "custom", unit: unit as "hour" | "day" });
    onClose();
  };
        ...
        >
          ...
          <PopoverBody px={0} py={0} ref={customRef}>
            <Tabs onChange={(index) => setTabIndex(index)} index={tabIndex}>
              <TabList px={3} pt={3}>
                <Tab fontSize={fontSize}>{t("userDialog.relative", "Relative")}</Tab>
                <Tab fontSize={fontSize}>{t("userDialog.absolute", "Absolute")}</Tab>
              </TabList>
              <TabPanels>
                <TabPanel px={3} pb={3}>
                  <VStack spacing={3} align="stretch">
                    {customFilterOptions.map((row) => (
                      <VStack key={row.title} alignItems="start" spacing={2}>
                        <Text fontSize={fontSize} fontWeight="medium" minW="60px">

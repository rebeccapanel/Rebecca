...
const computeRange = (amount: number, unit: Preset["unit"]): TimeRangeValue => {
  const end = dayjs().utc().endOf(unit === "hour" ? "hour" : "day");
  const start = end.subtract(amount - 1, unit).startOf(unit === "hour" ? "hour" : "day");
  return { start: start.toDate(), end: end.toDate() };
};

const toInput = (date: Date) => dayjs(date).format("YYYY-MM-DDTHH:mm");

const TimeRangePicker: FC<TimeRangePickerProps> = ({ presets, value, onChange }) => {
  ...
  const handleRelative = (amount: number, unit: Preset["unit"], key: string) => {
                    ...
                    <SimpleGrid columns={{ base: 3, sm: 6 }} spacing={2}>
                      {group.options.map((opt) => (
                        <Button
                          key={opt.key}
                          size="sm"
                          variant={activePreset === opt.key ? "solid" : "outline"}
                          colorScheme={activePreset === opt.key ? "primary" : undefined}
                          onClick={() => handleRelative(opt.amount, group.unit, opt.key)}
                        >
                          {opt.label}
                        </Button>
                      ))}

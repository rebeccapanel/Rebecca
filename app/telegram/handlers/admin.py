...
def get_logger_channel_id():
    settings = TelegramSettingsService.get_settings()
    return settings.logs_chat_id


def get_default_vless_flow():
    settings = TelegramSettingsService.get_settings()
    return settings.default_vless_flow
...
def help_command(message: types.Message):
    ...
    return bot.reply_to(
        message,
        """
{user_link} Welcome to Rebecca Telegram-Bot Admin Panel.
...
def edit_command(call: types.CallbackQuery):
    ...
    if db_user.status == UserStatus.on_hold:
        ...
        mem_store.set(
            f"{call.message.chat.id}:expire_date", datetime.fromtimestamp(db_user.expire) if db_user.expire else None
        )
...
<b>By :</b> <a href="tg://user?id={message.chat.id}">{message.from_user.full_name}</a>"""
            try:
                bot.send_message(get_logger_channel_id(), text, "HTML")
            except ApiTelegramException:
                pass
        ...
        with GetDB() as db:
            ...
            for protocol in xray.config.inbounds_by_protocol:
                if protocol in inbounds and protocol not in db_user.inbounds:
                    proxies.update(
                        {
                            protocol: {"flow": get_default_vless_flow()}
                            if get_default_vless_flow() and protocol == ProxyTypes.VLESS
                            else {}
                        }
                    )
                elif protocol in db_user.inbounds and protocol not in inbounds:
...
<b>New Proxies :</b> <code>{", ".join(list(user.inbounds.values())[0])}</code>{tag}"""
        ...
        original_proxies = {
            p: ({"flow": get_default_vless_flow()} if get_default_vless_flow() and p == ProxyTypes.VLESS else {})
            for p in inbounds
        }
...
<b>By :</b> <a href="tg://user?id={chat_id}">{full_name}</a>"""
                try:
                    bot.send_document(get_logger_channel_id(), open(file_name, "rb"), caption=text, parse_mode="HTML")
                    os.remove(file_name)
                except ApiTelegramException:
                    pass
    elif data == "add_data":
...
\t{user.status}\n"
            ...
            bot.send_message(chat_id, summary_text, "HTML", reply_markup=BotKeyboard.main_menu())
            if get_logger_channel_id():
                text = f"""\
📶 <b>#Traffic_Change #From_Bot</b>
➖➖➖➖➖➖➖➖➖

...
def save_user_usage_backup(updates: List[Dict[str, Any]]) -> bool:
    ...
    for update in existing:
        ...
        if user_id:
            ...
            if update.get('online_at') and (not merged[key]['online_at'] or update['online_at'] > merged[key]['online_at']):
        ...
        if user_snapshots or node_snapshots:
            for snapshot in user_snapshots:
                ...
                if user_id and created_at_str:
                    pending_key = f"{REDIS_KEY_PREFIX_USER_USAGE_PENDING}{user_id}:{node_id or 'master'}:{created_at_str}"
            ...
            logger.info(f"Restored {len(user_snapshots)} user snapshots and {len(node_snapshots)} node snapshots from backup")

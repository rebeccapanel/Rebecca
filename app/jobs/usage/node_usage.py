...
def record_node_stats(params: dict, node_id: Union[int, None]):
    ...
    with GetDB() as db:
        ...
        if node_id is not None and (total_up or total_down):
            ...
            limited_triggered, limit_cleared, status_change_payload = _update_master_limits(db, total_up, total_down)
...
def _persist_node_stats_in_session(db, params: list, node_id: Union[int, None], created_at):
    ...
    usage_query = db.query(NodeUsage).filter(and_(NodeUsage.node_id == node_id, NodeUsage.created_at == created_at))

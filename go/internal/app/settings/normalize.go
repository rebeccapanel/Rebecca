...
func appTemplateBasePath() string {
	...
	if cwd, err := os.Getwd(); err == nil {
		candidates = append(candidates, filepath.Join(cwd, "app", "templates"))
		candidates = append(candidates, filepath.Join(filepath.Dir(cwd), "app", "templates"))
		candidates = append(candidates, filepath.Join(filepath.Dir(filepath.Dir(cwd)), "app", "templates"))
	}
	if exe, err := os.Executable(); err == nil {
		dir := filepath.Dir(exe)
		candidates = append(candidates, filepath.Join(dir, "app", "templates"))
		candidates = append(candidates, filepath.Join(filepath.Dir(dir), "app", "templates"))
		candidates = append(candidates, filepath.Join(filepath.Dir(filepath.Dir(dir)), "app", "templates"))
	}
...
func displayTemplatePath(path string, templateName string, customDirectory *string) string {
	if customDirectory == nil || strings.TrimSpace(*customDirectory) == "" {
		return filepath.ToSlash(filepath.Join("app", "templates", filepath.Clean(templateName)))
	}
	return path
}
